...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, badgeCounts }) => {
  const navItems = [
    ...
    { id: 'integration', label: 'Auth Token (/api/auth/token)', icon: Key, isCustom: false },
  ];

  return (
    <>
      {/* Mobile Horizontal Navigation Strip */}
      <div className="md:hidden w-full overflow-x-auto py-2.5 px-3 border-b border-white/10 liquid-glass-subtle flex items-center gap-1.5 scrollbar-none z-30 sticky top-[65px] backdrop-blur-xl">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`shrink-0 flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-medium transition-all cursor-pointer ${
                isActive
      ...
      <aside className="hidden md:block w-64 liquid-glass-subtle border-r border-white/10 p-3.5 shrink-0 backdrop-blur-2xl">
